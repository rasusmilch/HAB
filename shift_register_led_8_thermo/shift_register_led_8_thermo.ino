...
const int clock = 9; //SCK
const int latch = 10; //RCK 
const int data = 8;  //DIO

const long THERMOCOUPLE_ERROR = -2147483648L;

// Example creating a thermocouple instance with software SPI on any three
// digital IO pins.
// MAX31855K x 8 multiplexing board.
#define MAXDO   5 // SO 
#define MAXCS   6 // CS
#define MAXCLK  7 // SCK
#define T0      2 // T0
...
byte letter_F = B10001110; // display F

// Array to hold thermocouple data.
long temps[8] = {0, 0, 0, 0, 0, 0, 0, 0};
...
long correct_temp() {
    ...
    if (isnan(rawTemp)) {
      ...
      correctedTemp = THERMOCOUPLE_ERROR;
    }
    else {
  ...
  //Serial.println();
  if (correctedTemp != THERMOCOUPLE_ERROR) {
...
for (int therm=0; therm<8; therm++) {
    ...
    /*if (c_int == THERMOCOUPLE_ERROR) {
      // Not connected, skip to next thermo couple.
  ...
  //  c_int = 0b111110;
    ...
    if (c_int < 537) {
      v7 = letter_F; // display F
    } else {
      next_digit(c_int, digit);
      v7 = digit;
    }
  
    // Edge case where 0 fahrenheit reading, but our modulus digit parser will believe it is the end of the parsing.
    if (c_int == 0) {
      digit = value[0];
    } else {
      next_digit(c_int, digit);
  ...
  for (int i = 0; i<8; i++) {
      ...
      //Serial.print(F(": F = "));
      if (temps[i] != THERMOCOUPLE_ERROR) {
        Serial.print(temps[i] * 0.018 + 32);
        Serial.print(F(" "));
      }
  }

...
struct {
    ...
    // Need to configure booleans to use a bitmap to save EEPROM. Arduino bools are simply 8 bit ints.
    bool DEBUG = false;
    bool DEBUG_MORE = false;
...
bool compare_buffers(uint8_t *first, uint8_t *second, uint16_t bytes) {
    ...
    for (uint16_t i = 0; i < bytes; i++) {
        if (first[i] != second[i]) {
            if (configuration.DEBUG_MORE == true) {
...
/*
    ...
    */
    if (configuration.alarm == false) {
        ...
        Serial.println(F("Controller will now configure sensors."));
        ...
        Serial.println(F("Please enter any commands now (or type help for command list.)"));
...
void heater_control() {
    ...
    if (dataLog.temp.external != -1000 && dataLog.temp.external != -127) {
    ...
    for (uint32_t block = 1; block <= sdcard_data.blocks; block++) {
        ...
        if (empty_block == false) {
            ...
            for (uint8_t i = 0; i < 510/data_size; i++) {
                ...
                Serial.print(dataLog.gps.time);
                Serial.print(F(","));
                Serial.print(dataLog.gps.lat, 10);
                Serial.print(F(","));
                Serial.print(dataLog.gps.lng, 10);
                Serial.print(F(","));
                Serial.print(dataLog.gps.altitude, 10);
                Serial.print(F(","));
                Serial.print(dataLog.gps.speed, 10);
                Serial.print(F(","));
                Serial.print(dataLog.gps.course, 10);
                Serial.print(F(","));
                Serial.print(dataLog.gps.sats);
                Serial.print(F(","));
                ...
                Serial.print(dataLog.tsl.ir);
                Serial.print(F(","));
                Serial.print(dataLog.uv, 10);
                Serial.print(F(","));
                Serial.print(dataLog.ms5607.pressure, 10);
                Serial.print(F(","));
                Serial.print(dataLog.sht31.humidity, 10);
                Serial.print(F(","));
                Serial.print(dataLog.accel.x, 10);
                Serial.print(F(","));
                Serial.print(dataLog.accel.y, 10);
                Serial.print(F(","));
                Serial.print(dataLog.accel.z, 10);
                Serial.print(F(","));
                Serial.print(dataLog.mag.x, 10);
                Serial.print(F(","));
                Serial.print(dataLog.mag.y, 10);
                Serial.print(F(","));
                Serial.print(dataLog.mag.z, 10);
                Serial.print(F(","));
                Serial.print(dataLog.gyro.x, 10);
                Serial.print(F(","));
                Serial.print(dataLog.gyro.y, 10);
                Serial.print(F(","));
                Serial.print(dataLog.gyro.z, 10);
                Serial.print(F(","));
                Serial.print(dataLog.temp.main, 10);
                Serial.print(F(","));
                Serial.print(dataLog.temp.internal, 10);
                Serial.print(F(","));
                Serial.print(dataLog.temp.external, 10);
                Serial.print(F(","));
                Serial.print(dataLog.temp.gps, 10);
                Serial.print(F(","));
                Serial.print(dataLog.temp.tsl2561, 10);
                Serial.print(F(","));
                Serial.print(dataLog.temp.sht31, 10);
                Serial.print(F(","));
                Serial.print(dataLog.temp.uv, 10);
                Serial.print(F(","));
                Serial.print(dataLog.temp.ms5607, 10);
                Serial.print(F(","));
                Serial.print(dataLog.temp.custom, 10);
                Serial.print(F(","));
                Serial.print(dataLog.temp.clock, 10);
                Serial.print(F(","));
                Serial.print(dataLog.geiger.x);
                Serial.print(F(","));
                Serial.print(dataLog.geiger.y);
                ...
                Serial.print(F(","));
                Serial.print(dataLog.battery.voltage, 10);
                Serial.print(F(","));
                Serial.print(dataLog.battery.soc, 10);
                Serial.print(F(","));
...
void print_serial_help() {
    ...
    Serial.println(F("\n\n\n\rCommand help list"));
    Serial.println(F("Commands NOT case sensitive"));
    Serial.println(F("============================================================================================="));
    Serial.println(F("help                    This command list.\n\r"));
    Serial.println(F("help read sdcard        Help with SDCard commands."));
    Serial.println(F("help set altitude       Help with altitude threshold commands."));
    Serial.println(F("help set pressure       Help with pressure threshold commands."));
    Serial.println(F("help set launch         Help with launch timer commands"));
    Serial.println(F("help set location       Help with location timer commands\n\r"));

    Serial.println(F("read sdcard             Output the SDCard data as a CSV to serial console.\n\r"));
    Serial.println(F("set altitude ###        Set GPS altitude threshold to ### meters."));
    Serial.println(F("set pressure ###        Set barometric pressure threshold to ### Pascals"));
    Serial.println(F("set launch ###          Set auto launch timer to ### minutes after power-on"));
    Serial.println(F("set location ###        Set auto location timer to ### minutes after power-on"));
    Serial.println(F("set dwell location ###  Set num of readings under altitude/pressure to trigger location mode."));
    Serial.println(F("============================================================================================="));
    //delay(1000);
}
    ...
    if (c == '\n' || c == '\r') {
        ...
        if (incoming.length() > 0) {
            ...
            if (incoming == "read sdcard") {
                read_sdcard();
            } else if (incoming == "debug on") {
                DEBUG = true;
                EEPROM.put(0, configuration);
            } else if (incoming == "debug off") {
                DEBUG = false;
                EEPROM.put(0, configuration);
